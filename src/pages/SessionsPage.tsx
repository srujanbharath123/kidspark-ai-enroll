        ...
        <div>
          ...
          {sessions.length === 0 ? (
            ...
            <div className="space-y-3">
              {sessions.map((s) => {
                const materials = sessionMaterials[s.id] || [];
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="flex-1">
                        ...
                        {s.meet_link ? (
                          ...
                          <p className="text-xs text-muted-foreground flex items-center gap-1 mt-1">
                            <Link2 className="w-3 h-3" /> Meeting link will appear here when the trainer shares it
                          </p>
                        ) : null}
                      </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-border/30">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {role === "trainer" ? (
                          ...
                          <Badge variant="outline" className="text-[10px]">{materials.length} item{materials.length > 1 ? "s" : ""}</Badge>
                        ) : null}
                      </div>
                      ...
                      {materials.length === 0 && role === "parent" ? (
                        <p className="text-xs text-muted-foreground">Trainer shared PDFs, videos, links, and notifications will appear here.</p>
                      ) : (
                        materials.map((m) => {
                          const materialUrl = m.file_url || (m.material_type === "link" && m.description?.trim().match(/^https?:\/\//i) ? m.description.trim() : null);
                          const hideDescription = m.material_type === "link" && materialUrl === m.description?.trim();

                          return (
                            <div key={m.id} className="flex items-start gap-2 py-2">
                              {m.material_type === "pdf" ? <FileText className="w-3.5 h-3.5 text-destructive shrink-0 mt-0.5" /> :
                               m.material_type === "video" ? <Video className="w-3.5 h-3.5 text-primary shrink-0 mt-0.5" /> :
                               m.material_type === "link" ? <Link2 className="w-3.5 h-3.5 text-secondary shrink-0 mt-0.5" /> :
                               <Bell className="w-3.5 h-3.5 text-accent shrink-0 mt-0.5" />}
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium truncate">{m.title}</p>
                                {m.description && !hideDescription && <p className="text-[10px] text-muted-foreground break-words">{m.description}</p>}
                              </div>
                              {materialUrl && (
                                <a href={materialUrl} target="_blank" rel="noreferrer" className="text-[10px] text-primary hover:underline shrink-0">Open</a>
                              )}
                              {role === "trainer" && (
